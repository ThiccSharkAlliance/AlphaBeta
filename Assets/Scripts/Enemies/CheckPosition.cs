...
public class CheckPosition : MonoBehaviour
{
    ...
    private float getPosition(float PosX, float PosZ, float ground, float scale, float groundScale)
    {
        ...
        float altitude = Noise.Generate2DNoiseValue(PosX, PosZ, scale, _engine.WorldInfo.Seed, ground);
    ...
    private bool validatePosition(float PosX, float PosZ, float ground, float scale, float groundScale)
    {
        ...
        float altitude = Noise.Generate2DNoiseValue(PosX, PosZ, scale, seed, ground);
        float groundAltitude = altitude - groundScale;
        float altitude2 = Noise.Generate2DNoiseValue(PosX + 0.5f, PosZ, scale, seed, ground);
        float groundAltitude2 = altitude - groundScale;
        float altitude3 = Noise.Generate2DNoiseValue(PosX - 0.5f, PosZ, scale, seed, ground);
        float groundAltitude3 = altitude3 - groundScale;
        float altitude4 = Noise.Generate2DNoiseValue(PosX, PosZ + 0.5f, scale, seed, ground);
        float groundAltitude4 = altitude4 - groundScale;
        float altitude5 = Noise.Generate2DNoiseValue(PosX, PosZ - 0.5f, scale, seed, ground);
